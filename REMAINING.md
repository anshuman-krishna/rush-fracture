...
- **No authentication anywhere.** Every endpoint (`POST /api/users`, `POST /api/runs/start`, `POST /api/runs/{id}/end`, etc.) is fully open. IDs are unguessable random strings but there's no ownership binding — anyone can create users or post runs/events/stats against any ID.
- **No input validation on gameplay data.** Score, level, kills, duration, combo, mutations, tags are persisted verbatim from client JSON with no bounds/type checks — the backend as-is cannot be trusted as an authoritative leaderboard source; any client can fabricate arbitrary stats.
- **No rate limiting, no request body size limits.** Combined with a single SQLite connection (`db.SetMaxOpenConns(1)`), this is a trivial DoS target.
- **CORS is wide open** (`Access-Control-Allow-Origin: *`), fine today only because nothing is authenticated; must be scoped before adding any credentialed/authenticated endpoint.
- **Websocket accepts any origin** (`CheckOrigin` always returns `true`) with no auth — low-impact today only because the broadcast path is dead code (see below).
- **Two features are silently non-functional**: the `stats` table is never written to (`GET /api/stats/{userId}` will 404 forever), and the websocket hub's `broadcast` channel is never sent to from anywhere — no real-time relay actually happens despite the message-type plumbing (`game_state`, `player_move`, `run_start`, `run_end`) existing in `websocket/message.go`.
- **No graceful shutdown, no structured logging, no DB migrations system** (schema is just `CREATE TABLE IF NOT EXISTS` at startup — any future schema change needs hand-written migration code), no health-check DB connectivity check, no TLS handling (assumed external, currently undocumented).
...
- **Two overlapping save files** (`player_profile.cfg` and `best_stats.cfg`) track near-duplicate stats independently and can drift out of sync — pick one source of truth.
- **Saves are written synchronously and non-atomically** on every currency/settings change (`config.save(PATH)` directly, no temp-file+rename). A crash or power loss mid-write can corrupt the file; on next load a corrupt/unreadable profile silently resets to a **fresh blank profile** — total loss of currency, unlocks, and stats, not just a field getting reset. This is a real refund/support-ticket generator once there are real players.
...
- **Heavy code duplication across 10 enemy controller files** (no shared base class — see CLAUDE.md) is a design-and-balance risk, not just a style issue: a balance change or bugfix applied to one enemy type must be manually re-applied to the other nine, and they've already begun to drift (e.g. inconsistent arena-boundary clamping — only 2 of 10 controller types clamp to the arena at all; inconsistent hit-flash implementations that mutate shared material resources in some files and not others).
