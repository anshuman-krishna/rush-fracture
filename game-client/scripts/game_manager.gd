...
# central coordinator. connects run, room, combat, fracture, and ui systems.
# does not own gameplay logic — delegates to specialized systems.

@onready var player: CharacterBody3D = $"../Player"
@onready var weapon: Node3D = $"../Player/Head/Weapon"
...
func _ready() -> void:
	...
	weapon.enemy_killed.connect(_on_weapon_kill)
	weapon.enemy_hit.connect(_on_weapon_hit)
	...
	upgrade_manager.bind(player, weapon)
...
func _on_room_enemy_killed() -> void:
	# check for enemy duplication fracture
